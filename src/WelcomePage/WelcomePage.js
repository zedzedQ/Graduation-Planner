...
class WelcomePage extends Component {
  ...
  onDragStart = start => {
    ...
    const highlightTerm = this.state.welcomeOrder[
      this.state.welcomeOrder.length - 1
    ];
  ...
  checkBeforeSubmit() {
    ...
    nextPage =
      ...
      (this.state.startTerm !== "" && this.state.startYear !== "");
  ...
  onDragEnd = result => {
    ...
    if (
      destination.droppableId !== course.category &&
      destination.droppableId !==
        this.state.welcomeOrder[this.state.welcomeOrder.length - 1]
    ) {
      return;
    }
    ...
    return (
      <div>
        ...
        >
          <Container>
            <AvailableCourses
              sections={this.state.welcomeOrder.slice(0, 7)}
              state={this.state}
            >
              {" "}
            </AvailableCourses>
            ...
            {this.state.welcomeOrder.slice(7).map((columnId, index) => {
              ...
              const courses = column.courseIds.map(
                courseId => this.state.cisCourses[courseId]
              );
